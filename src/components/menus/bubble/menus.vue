<template>
	...
	<template v-else-if="(is('image') && !attrs('image').error) || (is('inlineImage') && !attrs('inlineImage').error)">
		...
		<div class="umo-bubble-menu-divider"></div>
		<menus-bubble-image-preview v-if="attrs('image')?.type?.startsWith('image') || attrs('inlineImage')?.type?.startsWith('image')" />
		<menus-bubble-image-edit />
		<menus-bubble-image-open />
		<menus-bubble-node-duplicate v-if="is('image') && attrs('image').draggable" />
		<menus-bubble-node-tofile v-if="attrs('image').previewType !== null && attrs('inlineImage').previewType !== null" />
		<menus-bubble-image-convert />
		<div class="umo-bubble-menu-divider"></div>
		<menus-bubble-node-delete />
	</template>
	<template v-else-if="is('video') || is('audio') || is('file') || is('iframe')">
		<menus-toolbar-base-align-left />
		<menus-toolbar-base-align-center />
		<menus-toolbar-base-align-right />
		...
		<template v-if="is('iframe')">
			...
			<menus-toolbar-insert-web-page v-if="!disable('web-page')" ico="edit" :page-type="attrs('iframe')?.type" :page-url="attrs('iframe')?.src" />
			<menus-bubble-webpage-open />
			<div class="umo-bubble-menu-divider"></div>
		</template>
		<menus-bubble-file-download v-if="is('file') || is('video') || is('audio')" />
	...
	<template v-else-if="is('toc') || is('pageBreak') || is('horizontalRule') || is('codeBlock') || attrs('image').error">
		<!-- <menus-bubble-node-delete /> -->
	</template>
	<template v-else>
		<menus-toolbar-base-font-size :select="false" />
		<div v-if="!disable('font-size-increase') || !disable('font-size-decrease')" class="umo-bubble-menu-divider"></div>
		<menus-toolbar-base-bold />
		<menus-toolbar-base-italic />
