<template>
	<menus-button ico="table" :text="t('table.borderColor')" menu-type="popup" huge :disabled="!editor?.can().setCellAttribute('borderColor', '')" :popup-visible="popupVisible" @toggle-popup="togglePopup">
		<template #content>
			<t-space>
			...
			<t-space direction="vertical">
				<div v-for="item in options" :key="item.value">
					<t-space>
						<div>{{ item.label }}</div>
						<t-popup trigger="click" placement="right-bottom" :visible="item.popVisible">
							<t-tag theme="primary" :color="item.color" @click="togglePop(item)">{{ item.color }}</t-tag>
							<template #content>
								<t-card>
...
function setSelectionOuterBorderColor(side, color) {
	...
	if (!range) {
		...
		styleSelf = styleSelf.replace(new RegExp(`${propSelf}\\s*:[^;]+;?`, "gi"), "").trim();
		if (color) styleSelf += `${propSelf}: ${color};`;
		tr = tr.setNodeMarkup(cellPos, null, { ...cellNode.attrs, style: styleSelf || null });
		...
		if (side === "left") {
			...
			if (leftCell) {
				...
				tr = tr.setNodeMarkup(leftCell.pos, null, { ...leftCell.node.attrs, style: s || null });
			}
		}
		if (side === "top") {
			...
			if (topCell) {
				...
				tr = tr.setNodeMarkup(topCell.pos, null, { ...topCell.node.attrs, style: s || null });
			}
		}
		if (side === "right") {
			...
			if (rightCell) {
				...
				tr = tr.setNodeMarkup(rightCell.pos, null, { ...rightCell.node.attrs, style: s || null });
			}
		}
		if (side === "bottom") {
			...
			if (bottomCell) {
				...
				tr = tr.setNodeMarkup(bottomCell.pos, null, { ...bottomCell.node.attrs, style: s || null });
			}
		}
	...
	cells.forEach(({ pos, row, col, node }) => {
		...
		styleSelf = styleSelf.replace(new RegExp(`${propSelf}\\s*:[^;]+;?`, "gi"), "").trim();
		if (color) styleSelf += `${propSelf}: ${color};`;
		tr = tr.setNodeMarkup(pos, null, { ...node.attrs, style: styleSelf || null });
